...
export const EquipmentCard: React.FC<EquipmentCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {equipment.brand ? <InventoryMetaRow label="Marca" value={equipment.brand} /> : null}
      {equipment.model ? <InventoryMetaRow label="Modelo" value={equipment.model} /> : null}
      {equipment.serial_number ? (
        <InventoryMetaRow label="Número de serie" value={equipment.serial_number} mono />
      ) : null}
      {equipment.location ? <InventoryMetaRow label="Ubicación" value={equipment.location} /> : null}
      {equipment.assigned_to_user_name ? (
        <InventoryMetaRow label="Asignado a" value={equipment.assigned_to_user_name} />
      ) : null}
      {equipment.purchase_date && formatDate(equipment.purchase_date) ? (
        <InventoryMetaRow label="Fecha de compra" value={formatDate(equipment.purchase_date)} />
      ) : null}
    </InventoryCardShell>
  );
};
