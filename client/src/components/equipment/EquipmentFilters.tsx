...
interface EquipmentFiltersProps {
  ...
  onFilterChange: (key: keyof EquipmentFiltersType, value: EquipmentFiltersType[keyof EquipmentFiltersType]) => void;
  onSearch: () => void;
  onClearFilters: () => void;
  searchTerm: string;
  onSearchTermChange: (value: string) => void;
  types: Array<{ value: string; label: string; id?: number }>;
  statuses: Array<{ value: EquipmentStatus; label: string; color: string }>;
        ...
        <div className="sm:col-span-2 lg:col-span-1">
          ...
          <div className="flex">
            <input
              ...
              className="flex-1 px-4 py-2.5 border border-gray-300 rounded-l-lg rounded-r-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
            />
            <button
              onClick={onSearch}
              className="px-4 py-2.5 bg-blue-600 text-white rounded-r-lg rounded-l-none hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 flex items-center justify-center border border-blue-600 border-l-0 shadow-sm"
              type="button"
              aria-label="Buscar"
            >
              <svg
        ...
        {users && (
          <div>
            ...
            <select
              ...
              onChange={(e) => onFilterChange('assigned_to_user_id', e.target.value ? parseInt(e.target.value) : undefined)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              <option value="">Todos</option>
              {users.map((user) => (
